...
test('task should be added', () => {
  const endState = tasksReducer(
    ...
    createTask.fulfilled(
      ...
      '',
      { todolistId: 'todolistId2', title: 'apple' }
    )
  )
...
test('task should be removed', () => {
  const endState = tasksReducer(
    startState,
    deleteTask.fulfilled({ todolistId: 'todolistId1', taskId: '1' }, '', {
      todolistId: 'todolistId1',
      taskId: '1',
    })
  )
...
test('status of specified task should be changed', () => {
  const endState = tasksReducer(
    ...
    updateTaskStatus.fulfilled(
      { todolistId: 'todolistId2', status: TaskStatuses.New, taskId: '2' },
      '',
      { todolistId: 'todolistId2', taskId: '2', status: TaskStatuses.New }
    )
  )
...
test('title of task should be changed', () => {
  const endState = tasksReducer(
    startState,
    updateTaskTitle.fulfilled({ todolistId: 'todolistId2', taskId: '1', title: 'banana' }, '', {
      todolistId: 'todolistId2',
      taskId: '1',
      title: 'banana',
    })
...
test('new array should be added when new todolist is added', () => {
  const endState = tasksReducer(
    startState,
    createTodolist.fulfilled({ title: 'new todolist', todolistId: v1() }, '', 'new todolist')
  )
...
test('ids should be equals', () => {
  ...
  const action = createTodolist.fulfilled(
    { title: 'new todolist', todolistId: v1() },
    '',
    'new todolist'
  )
...
test('property with todolistId should be deleted', () => {
  const endState = tasksReducer(
    startState,
    deleteTodolist.fulfilled({ todolistId: 'todolistId2' }, '', 'todolistId2')
  )
